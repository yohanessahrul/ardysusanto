...
export function loginAction(email, password) {
  return dispatch => {
    axios.post('http://35.201.1.205:3001/api/user/login', {
      email: email,
      password: password
    })
      .then((response) => {
...
export function admingetAllUserAction() {
  return dispatch => {
    axios.get('http://35.201.1.205:3001/api/user/read')
      .then((response) => {
...
export function adminGetOneUserAction (id) {
  return dispatch => {
    axios.get(`http://35.201.1.205:3001/api/user/readbyid/${id}`)
      .then((response) => {
...
console.log('proses id', id)
  return dispatch => {
    axios.post(`http://35.201.1.205:3001/api/user/resetpassword/${id}`, { password: password, role: role })
      .then((response) => {
...
export function adminDeleteUserAction (id) {
  ...
  return dispatch => {
    axios.delete(`http://35.201.1.205:3001/api/user/delete/${id}`)
      .then((response) => {
...
export function adminEditRoleAction (id, newRole) {
  ...
  return dispatch => {
    axios.post(`http://35.201.1.205:3001/api/user/editrole/${id}`, { role: newRole })
      .then((response) => {
        if (response) {
          // console.log('----------------> ', response.data.data)
...
export function adminCreateUserAction (username, email, password) {
  ...
  return dispatch => {
    axios.post('http://35.201.1.205:3001/api/user/register', { username, email, password })
      .then((response) => {
        dispatch(adminGetAllUser(response.data.data))
        history.push('/admin/user')
...
export function cekAuthAction (token) {
  return dispatch => {
    axios.get(`http://35.201.1.205:3001/api/user/cekauth/${token}`)
      .then((response) => {
